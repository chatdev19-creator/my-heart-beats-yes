...
const Index = () => {
  ...
  const [noTransform, setNoTransform] = useState({
    x: 0,
    y: 0
  });
  ...
  return <main className="relative min-h-screen valentine-gradient-soft overflow-hidden font-body">
      <FloatingHearts />
      <ConfettiCanvas active={celebrating} duration={5000} onComplete={handleCelebrationComplete} />

      {/* Screen flash on celebration */}
      {screenFlash && <div className="fixed inset-0 z-[60] pointer-events-none bg-valentine-rose" style={{
      animation: "screen-flash 0.6s ease-out forwards"
    }} aria-hidden="true" />}

      {/* Background image overlay */}
      <div className="fixed inset-0 z-0 opacity-10 bg-cover bg-center pointer-events-none" style={{
      backgroundImage: `url(${valentineBg})`
    }} aria-hidden="true" />

      {/* Main content */}
      <div ref={containerRef} className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 py-12">
        {/* Heart icon */}
        <div className="animate-heartbeat mb-4">
          <Heart size={56} className="text-primary drop-shadow-lg" fill="currentColor" />
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 w-full max-w-sm">
          ...
          <button onClick={handleYes} disabled={buttonsDisabled} className={`
            ...
            `} style={{
          animation: !buttonsDisabled ? "valentine-pulse 2s ease-in-out infinite, neon-flicker 3s linear infinite" : "none"
      ...
      {/* Love letter modal */}
      <LoveLetter visible={showLetter} onClose={handleCloseLetter} isPlaying={isPlaying} onToggleMusic={toggleMusic} />
    </main>;
};
